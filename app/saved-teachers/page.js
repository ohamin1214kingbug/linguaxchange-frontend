    ...
    <main className="min-h-screen bg-cream">
      ...
      <div className="max-w-2xl mx-auto px-4 md:px-8 py-12">
          ...
          {found && (
            <div className="flex items-center justify-between gap-4 mt-4 p-4 bg-cream rounded-xl border-2 border-navy/10">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-bold text-navy">{found.first_name} {found.last_name}</p>
                  <p className="text-navy/50 text-xs">{LANGS[found.teach_language]?.flag} {LANGS[found.teach_language]?.name}</p>
                </div>
              </div>
        ...
        <div className="space-y-3">
          {saved.map(teacher => (
            <div key={teacher.id} className="bg-white rounded-2xl p-4 border-2 border-navy flex items-center justify-between gap-4">
              <a href={`/teachers/${teacher.id}`} className="flex items-center gap-3 hover:opacity-80 transition-opacity">
                {teacher.photo_url ? (
                  <img src={teacher.photo_url} alt={teacher.first_name} className="w-12 h-12 rounded-full object-cover border-2 border-navy"/>
                ) : (
                  <div className="w-12 h-12 rounded-full bg-brand-red flex items-center justify-center text-white font-bold border-2 border-navy">
                    {teacher.first_name?.[0]?.toUpperCase()}
                  </div>
                )}
                <div>
                  <p className="font-bold text-navy">{teacher.first_name} {teacher.last_name}</p>
